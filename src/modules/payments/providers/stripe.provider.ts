...
export class StripeProvider implements PaymentGatewayProvider {
  ...
  async initializePayment(
    ...
    const successUrl = input.successUrl ?? 'https://example.com/payment/success';
    ...
    form.append('line_items[0][price_data][currency]', payment.currency.toLowerCase());
    form.append('line_items[0][price_data][unit_amount]', `${Math.round(payment.amount * 100)}`);
    form.append('line_items[0][price_data][product_data][name]', `Order ${payment.orderId}`);
    form.append('metadata[paymentId]', payment.id);
    form.append('metadata[orderId]', payment.orderId);

    const response = await axios.post(`${this.baseUrl}/checkout/sessions`, form, {
      headers: {
